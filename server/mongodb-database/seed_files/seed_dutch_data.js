...
const filePath = path.join(__dirname, '../../../data_files/generated_dutch_sentences.txt');
...
async function readFullDataFile(filePath) {
  ...
  for await (const line of rl) {
    ...
    try {
      ...
      const wordObj = { index, englishWord, dutchWord, definitions, sentences };
...
// Insert into MongoDB
async function seedDutchData() {
  await connectDB();

  const dutchData = await readFullDataFile(filePath).catch(err => {
    console.error('Error reading file:', err);
    return []; // return empty array so code continues
  });

  // Transform for MongoDB schema
  const documents = dutchData.map(obj => ({
    wordIndex: obj.index,
    englishWord: obj.englishWord,
    dutchWord: obj.dutchWord,
    dutDef1: obj.definitions[0],
    dutDef2: obj.definitions[1] || '',
    dutDef3: obj.definitions[2] || '',
    dutSentence1: obj.sentences[0],
    dutSentence2: obj.sentences[1] || '',
    dutSentence3: obj.sentences[2] || '',
  }));

  const result = await DutchSentence.insertMany(documents);
...
export {filePath, readFullDataFile, seedDutchData}
