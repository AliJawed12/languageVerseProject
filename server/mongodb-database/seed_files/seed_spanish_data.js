...
const filePath = path.join(__dirname, '../../../data_files/generated_spanish_sentences.txt');
...
async function readFullDataFile(filePath) {
  ...
  for await (const line of rl) {
    ...
    try {
      ...
      const spanishWord = parts[2].trim();
      ...
      const wordObj = { index, englishWord, spanishWord, definitions, sentences };
...
async function seedSpanishData() {
  ...
  const spanishData = await readFullDataFile(filePath).catch(err => {
    console.error('Error reading file:', err);
    return []; // return empty array so code continues
  });

  // Transform for MongoDB schema
  const documents = spanishData.map(obj => ({
    wordIndex: obj.index,
    englishWord: obj.englishWord,
    spanishWord: obj.spanishWord,
    spaDef1: obj.definitions[0],
    spaDef2: obj.definitions[1] || '',
    spaDef3: obj.definitions[2] || '',
    spaSentence1: obj.sentences[0],
    spaSentence2: obj.sentences[1] || '',
    spaSentence3: obj.sentences[2] || '',
  }));

  const result = await SpanishSentence.insertMany(documents);
...
export {filePath, readFullDataFile, seedSpanishData}
